...
class JqgridTagLib {
    ...
    def jqgrid = { attrs, body ->
        ...
        if (!attrs.showFirstColumn || Boolean.parseBoolean(attrs.showFirstColumn)) {
            colModel << [name: "id", index: "id", width: attrs.firstColumnWidth ?: 40, formatter: showFormatter]
            colNames << ""
        }
        ...
        if (attrs.columns) {
            columns.addAll(attrs.columns)
        }
        if (columns) {
            columns.each {
                //colNames << it.name
                colNames << message(code: "${domainClass.propertyName}.${it.name}", default: it.name)
                def width = ((columnProperties[it.name]?.width) ?: it.width) ?: 100
        ...
        if (columns) {
            def jsonColumns = ("" + (columns as JSON)).replaceAll("\"", "'")
    ...
    def column = { attrs, body ->
        def columns = request.getAttribute("columns")
        def column = [:]
        column.name = attrs.name
        if (attrs.width)
            column.width = attrs.width
        if (attrs.formatter)
            column.formatter = attrs.formatter
        if (attrs.formatoptions)
            column.formatoptions = attrs.formatoptions
        if (attrs.expression)
            column.expression = attrs.expression
        columns << column
    }
    ...
    def command = { attrs, body ->
        def resp = [icon: attrs.icon]
        if (attrs.handler)
            resp.handler = attrs.handler
        if (attrs.title)
            resp.title = attrs.title
        if (attrs.controller)
            resp.controller = attrs.controller
        if (attrs.action)
